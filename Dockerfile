...
RUN wget --no-verbose --no-check-certificate https://download3.rstudio.org/ubuntu-14.04/x86_64/VERSION -O "version.txt" && \
    ...
    . /etc/environment && \
    R -e ".libPaths('/usr/local/lib/R/site-library/'); install.packages(c('shiny', 'devtools', 'rmarkdown'), repos='$MRAN')" && \
...
RUN R -e ".libPaths('/usr/local/lib/R/site-library/'); library(devtools); devtools::install_dev_deps('/pkgs/dockerr.dashboard')"
